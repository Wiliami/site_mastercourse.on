...
(function (global, factory) {
  ...
  function getOppositePlacement(placement) {
    return placement.replaceAll(/left|right|bottom|top/g, matched => hash$1[matched])
  }
  ...
  function getOppositeVariationPlacement(placement) {
    return placement.replaceAll(/start|end/g, matched => hash[matched])
  }
  ...
  */
  ...
  function getClippingRect(element, boundary, rootBoundary) {
    const mainClippingParents = boundary === 'clippingParents' ? getClippingParents(element) : [boundary].flat()
  ...
  function detectOverflow(state, options) {
    ...
    const paddingObject = mergePaddingObject(typeof padding === 'number' ? expandToHashMap(padding, basePlacements) : padding)
    ...
    if (makeFallbackChecks) {
      ...
      const _loop = function _loop(_i) {
        const fittingPlacement = placements.find(placement => {
          ...
          if (checks) {
            return checks.slice(0, _i).every(Boolean)
          }
        })
  ...
  function mergeByName(modifiers) {
    const merged = modifiers.reduce((merged, current) => {
      ...
      merged[current.name] = existing
        ? ({ ...existing, ...current, options: { ...existing.options, ...current.options },
  ...
  class Dropdown extends BaseComponent {
    ...
    show() {
      ...
      if ('ontouchstart' in document.documentElement && !parent.closest(SELECTOR_NAVBAR_NAV)) {
        document.body.children.flat().forEach(elem => EventHandler.on(elem, 'mouseover', noop))
      }
    ...
    _completeHide(relatedTarget) {
      ...
      if ('ontouchstart' in document.documentElement) {
        document.body.children.flat().forEach(elem => EventHandler.off(elem, 'mouseover', noop))
      }
  ...
  class Tooltip extends BaseComponent {
    ...
    hide() {
      ...
      if ('ontouchstart' in document.documentElement) {
        document.body.children.flat().forEach(element => EventHandler.off(element, 'mouseover', noop))
      }
